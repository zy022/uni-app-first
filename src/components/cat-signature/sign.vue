<template>
  <view v-if="visibleSync" class="cat-signature" :class="{ visible: show }" @touchmove.stop.prevent="moveHandle">
    ...
    <view class="content">
      <canvas class="firstCanvas" :canvas-id="canvasId" @touchmove="move" @touchstart="start($event)" @touchend="end" @touchcancel="cancel" @longtap="tap" disable-scroll="true" @error="error" />
      <view class="btns">
        <view class="btn" @tap="clear">取消</view>
        <view class="btn btn-blue" @tap="save">保存</view>
      </view>
    </view>
  </view>
</template>
...
.btn-blue {
  background-color: #05ca99 !important;
